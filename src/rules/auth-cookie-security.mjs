...
export default {
  ...
  check(file) {
    ...
    lines.forEach((raw, idx) => {
      if (!/set-cookie/i.test(raw)) return;
      ...
      findings.push({
        line: idx + 1,
        column: raw.search(/set-cookie/i) + 1,
        message: 'A Set-Cookie value is written without the HttpOnly attribute — the cookie is readable by JavaScript.',
